...
void _reset(void)
{
	uint8_t *p2Sdata_flash = (uint8_t*)&_eText;
	uint8_t *p2Sdata_sram  = (uint8_t*)&_sSRAM;
	uint8_t *p2Edata_sram  = (uint8_t*)&_eData;
	uint8_t *p2Sbss_sram   = (uint8_t*)&_sBSS;
	uint8_t *p2Ebss_sram   = (uint8_t*)&_eBSS;

	while(p2Sdata_sram < p2Edata_sram)
	{
		*p2Sdata_sram = *p2Sdata_flash;
		((uint8_t*)p2Sdata_flash++);
		((uint8_t*)p2Sdata_sram++);
	}

	while(p2Sbss_sram < p2Ebss_sram)
	{
		*p2Sbss_sram = 0;
		p2Sbss_sram++;
	}
...
uint32_t _IVT[] __attribute__((section(".vectors"))) =
{
		(uint32_t)&_stack_top,
		(uint32_t)&_reset,
